...
export default function OMCComprehensiveDashboard() {
    ...
    const fetchData = async () => {
      ...
      const timeoutId = setTimeout(async () => {
        try {
          ...
          console.log('OMC API Request params:', {
            startDate,
            endDate,
            selectedCompanies: selectedCompanies.length,
            selectedProducts: selectedProducts.length,
            url: `http://localhost:8003/api/v2/omc/comprehensive?${params}`
          });

          const response = await fetch(`http://localhost:8003/api/v2/omc/comprehensive?${params}`, {
          ...
          console.log('OMC API Response summary:', {
          ...
          if (err.name === 'AbortError') {
            console.log('OMC Request was cancelled');
          } else {
            setError(err.message || 'Failed to load OMC comprehensive analytics');
            console.error('Error fetching OMC comprehensive data:', err);
          }
        } finally {
          setLoading(false);
        }
  ...
  if (loading) return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-gray-400">Loading OMC comprehensive analytics...</div>
      </div>
    </div>
  );
  ...
  // Market share distribution - show top N companies plus "Others"
  ...
  const othersShare = remainingCompanies.reduce((sum, company) => sum + company.market_share, 0);
  ...
  const marketShareData = [
    ...displayedCompanies.map(company => ({
      ...
      avgVolumePerProduct: company.efficiency_ratio,
  ...
  const seasonalData = Array.from({ length: 12 }, (_, i) => {
    ...
    if (month === 7 || month === 8) {
      console.log(`OMC Month ${month} debug:`, {
        monthData: monthData,
    ...
    const totalVolume = monthData.reduce((sum, t) => {
      ...
      if (month === 7 || month === 8) {
        console.log(`  OMC Adding volume for ${t.year}-${t.month}: ${vol}`);
      }
      return sum + vol;
    }, 0);
    ...
    if (month === 7 || month === 8) {
      console.log(`OMC Month ${month} result: totalVolume=${totalVolume}, avgVolume=${avgVolume}, dataPoints=${monthData.length}`);
    }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white">OMC Comprehensive Analytics</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
            ...
            {/* Market Share Legend */}
            <div className="mt-4 space-y-3">
              <div className="text-sm font-medium text-gray-300">Volume Distribution by Company:</div>
